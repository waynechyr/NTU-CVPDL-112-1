...
for image_info in coco['images']:
    if all(selected_per_category[cat_id] >= 20 for cat_id in categories.keys()):
        break  # Stop if 20 images are selected for each category
    ...
    if len(boxes) <= 6 and len(set(labels)) == 1:
        selected_per_category[ann['category_id']] += 1
...
# Output to JSON file
with open('/data/CVPDL3/output2.json', 'w') as json_file:
